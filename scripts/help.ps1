...
Write-Cmd 'task allegato' 'anteprima e verifica di ALLEGATO-TECNICO.md'
...
Write-Note 'allegato: compila l''anteprima e controlla cosa manca prima della consegna.'
...
Write-Cmd 'task import-service SRC=... [PORT=...]' 'importa un semilavorato/mock fornito'
Write-Cmd 'task new-entity SERVICE=<modulo> NAME=<Entita>' 'entity, repository, service, controller'
Write-Cmd 'task add-relation SERVICE=<modulo> FROM=... TO=...' 'relazione JPA fra due entity (@ManyToOne, ecc.)'
Write-Cmd 'task new-dto NAME=<Nome> FIELDS=<campi>' 'record/DTO in common-dto per Feign e REST'
Write-Cmd 'task new-client FROM=<mod1> TO=<mod2>' 'client OpenFeign (con auto-generazione DTO)'
Write-Cmd 'task new-view SERVICE=<modulo-ui> NAME=<Nome>' 'controller Thymeleaf e pagina HTML con form'
Write-Cmd 'task new-auth SERVICE=<modulo> [TYPE=...]' 'sicurezza e utenti (inmemory, db, o form)'
Write-Cmd 'task new-handler SERVICE=<modulo>' 'gestore eccezioni globale (@RestControllerAdvice)'
...
Write-Cmd 'task set-package PACKAGE=<base>' 'cambia il pacchetto Java di tutti i moduli'
Write-Cmd 'task set-java' 'allinea Java al JDK di questa macchina'
...
Write-Note 'new-entity: FIELDS=..., DTO=1 per usare i DTO (task --summary new-entity)'
Write-Note 'add-relation: interattivo se senza parametri, o TYPE=many-to-one|one-to-many|...'
Write-Note 'new-dto: FIELDS=id:long,titolo:string:required (task --summary new-dto)'
Write-Note 'new-client: DTO=<NomeDto> FIELDS=<campi> (task --summary new-client)'
Write-Note 'new-view: FIELDS=nome:string,prezzo:decimal (task --summary new-view)'
Write-Note 'new-auth: TYPE=inmemory (REST), TYPE=db (Entity/Repo/BCrypt), TYPE=form (login.html)'
Write-Note 'use-postgres: DBNAME=<db>, REMOVE_H2=1 per togliere h2 dal pom.'
