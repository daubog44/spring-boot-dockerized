...
cmd "task allegato" "anteprima e verifica di ALLEGATO-TECNICO.md"
...
note "allegato: compila l'anteprima e controlla cosa manca prima della consegna."
...
cmd "task import-service SRC=... [PORT=...]" "importa un semilavorato/mock fornito"
cmd "task new-entity SERVICE=<modulo> NAME=<Entita>" "entity, repository, service, controller"
cmd "task add-relation SERVICE=<modulo> FROM=... TO=..." "relazione JPA fra due entity (@ManyToOne, ecc.)"
cmd "task new-dto NAME=<Nome> FIELDS=<campi>" "record/DTO in common-dto per Feign e REST"
cmd "task new-client FROM=<mod1> TO=<mod2>" "client OpenFeign (con auto-rilevamento rotta/DTO)"
cmd "task new-view SERVICE=<modulo-ui> NAME=<Nome>" "controller Thymeleaf e pagina HTML con form"
cmd "task new-auth SERVICE=<modulo> [TYPE=...]" "sicurezza e utenti (inmemory, db, o form)"
cmd "task new-handler SERVICE=<modulo>" "gestore eccezioni globale (@RestControllerAdvice)"
...
note "new-entity: FIELDS=..., DTO=1 per usare i DTO (task --summary new-entity)"
note "add-relation: interattivo se senza parametri, o TYPE=many-to-one|one-to-many|..."
note "new-dto: FIELDS=id:long,titolo:string:required (task --summary new-dto)"
note "new-client: ROUTE=/api/... NAME=... DTO=... (task --summary new-client)"
note "new-view: FIELDS=nome:string,prezzo:decimal (task --summary new-view)"
note "new-auth: TYPE=inmemory (REST), TYPE=db (Entity/Repo/BCrypt), TYPE=form (login.html)"
note "use-postgres: DBNAME=<db>, REMOVE_H2=1 per togliere h2 dal pom."
